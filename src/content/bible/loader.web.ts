...
/**
 * Web build. Books are served as static files from `public/bible/kjv/` and
 * fetched on demand, so ~5 MB of Scripture never enters the JS bundle and
 * cannot delay first paint. Once fetched a book is cached for the session, and
 * the browser caches the file itself beyond that.
 */
...
export async function loadBook(id: BookId): Promise<BibleBook> {
  ...
  const request = (async () => {
    const res = await fetch(`/bible/kjv/${id}.json`);
